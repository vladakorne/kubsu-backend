...
<body>
    ...
    foreach ($result as $row) {
        ...
        echo "<td>" . htmlspecialchars(strip_tags($row['count'])) . "</td>";
        echo "</tr>";
    }
            ...
            <table class="table table-bordered">
                ...
                foreach ($result as $res) {
                    ...
                    <tr>
                        ...
                        <td><input disabled type="text" name="fio[]" class="form-control" value="<?= htmlspecialchars(strip_tags($res['name'])) ?>"></td>
                        <td><input disabled type="text" name="tel[]" class="form-control" value="<?= htmlspecialchars(strip_tags($res['tel'])) ?>"></td>
                        <td><input disabled type="email" name="email[]" class="form-control" value="<?= htmlspecialchars(strip_tags($res['email'])) ?>"></td>
                        <td><input disabled type="date" name="date_of_birth[]" class="form-control" value="<?= htmlspecialchars(strip_tags($res['date_of_birth'])) ?>"></td>
                        <td>
                        ...
                        <td>
                            <select disabled multiple name="languages<?= $counter ?>[]" class="form-control">
                                ...
                                foreach ($pLangs as $pLang) {
                                    echo '<option value="' . htmlspecialchars(strip_tags($pLang['id'])) . '"' . (in_array($pLang['id'], $langs) ? ' selected' : '') . '>' . htmlspecialchars(strip_tags($pLang['title'])) . '</option>';
                                }
                                ?>
                            </select>
                        </td>
                        <td><textarea disabled name="bio[]" rows="3" class="form-control"><?= htmlspecialchars(strip_tags($res['bio'])) ?></textarea></td>
                        <td><?= htmlspecialchars(strip_tags($res['user_id'])) ?></td>
                        <td>
                            <form action="" method="POST">
                                <input type="hidden" name="login" value="<?= htmlspecialchars(strip_tags($res['user'])) ?>">
                                <input type="hidden" name="uid" value="<?= htmlspecialchars(strip_tags($res['user_id'])) ?>">
                                <input type="hidden" name="id" value="<?= htmlspecialchars(strip_tags($res['id'])) ?>">
